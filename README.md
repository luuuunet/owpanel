...
```bash
curl -fsSL https://cdn.jsdelivr.net/gh/luuuunet/open-panel@v0.1.6/scripts/install.sh | sudo bash
```

Force source build (slow, 15–30 min on small VPS):

```bash
FROM_SOURCE=1 curl -fsSL https://cdn.jsdelivr.net/gh/luuuunet/open-panel@v0.1.6/scripts/install.sh | sudo bash
```
